...
import { generatePrefixedId, normalizeNZPhone } from "../util/util.js";
...
export const updateCustomerContact = async (req, res) => {
  ...
  try {
    ...
    await createChangeLogSafe({
      ...
      user_uuid: actorUserUuid,
      action: "update",
      summary: `Updated customer contact ${updated.first_name}${updated.last_name ? ` ${updated.last_name}` : ""}`,
    ...
    return res.status(500).json({ error: err.message || "Failed to update customer contact" });
  }
};
...
export const deleteCustomerContact = async (req, res) => {
  ...
  try {
    ...
    const deleted = await CustomerContact.softDeleteByUUID(uuid);
    ...
    await createChangeLogSafe({
      ...
      user_uuid: actorUserUuid,
      action: "delete",
      summary: `Deleted customer contact ${existing.first_name}${existing.last_name ? ` ${existing.last_name}` : ""}`,
      changed_fields: {
        is_deleted: true,
    ...
    return res.status(500).json({ error: err.message || "Failed to delete customer contact" });
  }
};
