...
import { generateShortId, verifyEmailToken, generateEmailToken, formatFullName } from '../util/util.js';
...
export const registerUser = async (req, res) => {
  const { email, password, firstName, lastName, role, customerUuid } = req.body;
  let authUser = null;
  let user = null;
  let customer = null;
  let employee = null;
  ...
  try {
    // ===== Customer UUID check =====
    if (customerUuid) {
      const existingCustomer = await Customer.findByUUID(customerUuid);
      if (!existingCustomer) return res.status(400).json({ error: "Invalid customer UUID" });
    }

    // ===== Check existing user in Supabase Auth =====
    const { data: existingUsers } = await supabase.auth.admin.listUsers({ email: normalizedEmail });
    if (existingUsers?.length) return res.status(409).json({ error: "Email already registered" });
    ...
    // 3a️⃣ Automatically create associated customer record if role = customer
    if (finalRole === 'customer') {
      ...
      customer = await Customer.create({
        ...
        last_name: lastName.toLowerCase(),
        email: normalizedEmail,
        user_uuid: user.uuid, // link to local user
      });
    }

    // 3b️⃣ Automatically create employee record if role = employee, admin, or owner
    if (["employee", "admin", "owner"].includes(finalRole)) {
      let employeeUUID;
      let existsEmployee;
      do {
        employeeUUID = generateShortId(9);
        existsEmployee = await Employee.findByUUID(employeeUUID);
      } while (existsEmployee);

      employee = await Employee.create({
        uuid: employeeUUID,
        user_uuid: user.uuid,
        // optional: other fields can be filled later by admin/owner
      });
    }

    // 4️⃣ Create email verification token
    const emailToken = jwt.sign(
      { user_uuid: user.uuid, auth_user_id: authUser.id, purpose: 'email_verification' },
    ...
    // 5️⃣ Mark confirmation sent in Supabase
    ...
    if (authUser?.id) {
      try { await supabase.auth.admin.deleteUser(authUser.id); } 
      catch (cleanupErr) { console.error('FAILED TO ROLLBACK AUTH USER', cleanupErr); }
    }

    if (user?.uuid) {
      try { await User.hardDeleteUserLocally(user.uuid); } 
      catch (cleanupErr) { console.error('FAILED TO ROLLBACK LOCAL USER', cleanupErr); }
    }
...
//   if (process.env.NODE_ENV !== "production") {
//     const allowedRoles = ["admin", "staff", "customer", "owner"];
...
// export const verifyEmail = async (req, res) => {
//   const { token } = req.body; // or req.params.token if using URL param
...
//     // 1️⃣ Update Supabase Auth
//     await supabase.auth.admin.updateUserById(user.auth_user_id, {
//       email_confirm: new Date().toISOString()
//     });
...
export const login = async (req, res) => {
  ...
  try {
    ...
    const updatedUser = await User.updateByUUID(user.uuid, { last_logged_in_at: new Date().toISOString() });
