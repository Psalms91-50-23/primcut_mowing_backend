...
import { generateShortId, formatFullName, EMAIL_TOKEN_EXPIRES_IN, getClientIp, generatePrefixedId } from '../util/util.js';
...
export const registerUser = async (req, res) => {
  ...
  // NEW: track whether we created a new customer during this request
  ...
  try {
    ...
    // 3️⃣ If role is customer: link to existing customer by email OR create new
    if (finalRole === "customer") {
      if (!resolvedCustomer) {
      ...
      if (!resolvedCustomer) {
        ...
        // NEW: mark that we created this customer and should rollback if later steps fail
        customerCreated = true;
        createdCustomerUuid = resolvedCustomer.uuid;
      } else {
        // Optional: enrich existing customer record without overwriting good data
        try {
          await Customer.updateByUUID(resolvedCustomer.uuid, {
            first_name: resolvedCustomer.first_name || firstName.toLowerCase(),
            last_name: resolvedCustomer.last_name || lastName.toLowerCase(),
          });
        } catch (e) {}
      }
    }

    // 4️⃣ Create user in local DB
    user = await User.create({
      auth_user_id: authUser.id,
      email: authUser.email,
      first_name: firstName.toLowerCase(),
    ...
    // 6️⃣ Mark confirmation sent in Supabase
    const { data: authUserRpc, error: rpcError } = await supabase.rpc(
    ...
    // 🔄 NEW: Rollback customer ONLY if we created it in this request
    if (customerCreated && createdCustomerUuid) {
      try {
        // Optional safety: only delete if safe (no quotes/jobs). If you have this helper, use it.
        // await Customer.safeDeleteByUUID(createdCustomerUuid);
...
export const login = async (req, res) => {
  ...
  try {
    ...
    const debugByEmail = await User.findByEmail(
...
export const createUserEmptyEmployee = async (req, res) => {
  ...
  try {
    ...
    do {
      ...
      employeeUuid = generatePrefixedId(9, "E");
